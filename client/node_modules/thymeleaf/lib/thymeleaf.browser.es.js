...
}), new ThymeleafRule('TemplateName', /[\w-._/]+/), new ThymeleafRule('FragmentName', /[\w-._]+/), new ThymeleafRule('FragmentParametersSection', Sequence(/\(/, Optional('FragmentParameters'), /\)/), ([, parameters]) => context => {
...
class ExpressionProcessor {
  ...
  process(input, context = {}) {
    ...
    return expression ? expression(_objectSpread({}, context, {
      expression: input
    })) : null;
  }
  ...
  process(element, context) {
    ...
    while (element.firstChild) {
      ...
      if (child instanceof Element && element.__thymeleafLocalVariables) {
        child.__thymeleafLocalVariables = _objectSpread({}, child.__thymeleafLocalVariables || {}, element.__thymeleafLocalVariables);
      }
    }
...
function deserialize$1(htmlString) {
...
function serialize$1(documentFragment) {
...
async function extractFragment(fragmentInfo, context) {
  ...
  if (templateResolver) {
    ...
    let template = deserialize$1((await templateResolver(templateName)));
    ...
    return $(`[${dialectPrefix}\\:${fragmentProcessorName}^="${fragmentName}"]`, template) || $(`[data-${dialectPrefix}-${fragmentProcessorName}^="${fragmentName}"]`, template);
  }
...
const STANDARD_CONFIGURATION = _objectSpread({}, DEFAULT_CONFIGURATION, {
...
class TemplateEngine {
  ...
  constructor({
    ...
    this.isomorphic = isomorphic;
    this.templateResolver = templateResolver; // Combine all processors into a unified list
    ...
    this.expressionObjects = dialects.reduce((acc, {
      expressionObjects
    }) => expressionObjects ? _objectSpread({}, acc, expressionObjects) : acc, {});
  }
  /**
  ...
  process(template, context = {}) {
    let document = deserialize$1(template);
    let rootElement = document.firstElementChild;
    return this.processNode(rootElement, _objectSpread({}, context, this.expressionObjects, {
  ...
  async processNode(element, context = {}) {
    ...
    let localContext = _objectSpread({}, context, localVariables);
...
export { STANDARD_CONFIGURATION, TemplateEngine, Dialect, AttributeProcessor, ExpressionProcessor, StandardDialect };
